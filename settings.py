...
class SettingsWindow(tk.Toplevel):
    ...
    def __init__(self,settings):
        ...
        self.basic,self.see_fit,self.zoom = tk.IntVar(value=settings[-1]),tk.IntVar(value=settings[-2]),tk.IntVar(value=settings[-3])
        ...
        tk.Button(self,text='Valid',bg='light blue',command=self.valid_settings).place(x=210,y=260)
        tk.Checkbutton(self,text='Remove only steep artifacts',variable=self.basic,bg='light grey').place(x=20,y=280)
        self.mainloop() 
        
    def valid_settings(self): # save settings, close settings window
        self.settings[:] = [setting.get() for setting in self.settings_new]+[self.zoom.get(),self.see_fit.get(),self.basic.get()]
